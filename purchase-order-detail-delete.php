...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    $sql_get_order_id = "SELECT * FROM purchase_order_details WHERE id = $get_id";
    if($result_order = mysqli_query($link,$sql_get_order_id)){
        if(mysqli_num_rows($result_order)){
            while($row_order = mysqli_fetch_assoc($result_order)){
                echo $get_order_id = $row_order['order_id'];
            }
        } 
    }

    // Prepare a delete statement
    $sql = "DELETE FROM purchase_order_details WHERE id = ?";
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            header("location: purchase-order-read.php?order_id={$get_order_id}");
            exit();
        } else{
...
<head>
    <meta charset="UTF-8">
    <title>Delete Purchase Order Info</title>
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        ...
                        <h4 class="alert-heading">Delete Item</h4>
                            ...
                            if($result_order = mysqli_query($link,$sql_get_order_id)){
                                if(mysqli_num_rows($result_order)){
                                    while($row_order = mysqli_fetch_assoc($result_order)){
                                         $get_order_id = $row_order['order_id'];
                                    }
                                } 
                            }
                            ?>
                                <input type="submit" value="Yes" class="btn btn-danger">
                                <a href="purchase-order-read.php?order_id=<?php echo $get_order_id;?>" class="btn btn-primary">No</a>
                            </p>
                        </div>
                    </form>
                </div>
